...
A simpler hit-rate script is also available: `python scripts/evaluate_rag.py` — 15 QA pairs over all 4 policy documents, hit rate @ k=3. Run after `python -m scripts.index_documents`.

| Metric | Result |
|---|---|
| Hit rate @ k=3 | **14/15 (93%)** |
| Avg latency (warm) | ~140 ms |
| Avg latency (cold start) | ~3.8 s (first query after index load) |

The one miss: *"What must happen within 2 business days after an emergency purchase?"* — top-3 chunks were from `exception-handling-procedure` but the specific 2-business-day clause landed just outside the window. The section-level evaluation above (which uses the stricter `evaluate_retrieval.py` suite) catches this pattern and shows hybrid BM25+semantic closes most such gaps on the primary 10-question suite.
